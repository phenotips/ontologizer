...
public class ProbabilisticCalculationTest
{
    ...
    public void testCalculateStudySet()
    {
        ...
        for (int i = 0; i < 10; i++) {
            assocContainer
                .addAssociation(new Association(new ByteString("gene" + i), new TermID(TermID.DEFAULT_PREFIX, 2)));
        }

        /* for C3 */
        for (int i = 10; i < 20; i++) {
            assocContainer
                .addAssociation(new Association(new ByteString("gene" + i), new TermID(TermID.DEFAULT_PREFIX, 3)));
        }

        /* for C4 */
        for (int i = 20; i < 100; i++) {
            assocContainer
                .addAssociation(new Association(new ByteString("gene" + i), new TermID(TermID.DEFAULT_PREFIX, 4)));
        }
