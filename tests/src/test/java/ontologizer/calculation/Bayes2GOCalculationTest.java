...
public class Bayes2GOCalculationTest
{
    ...
    public static class SingleCalculationSetting
    {
         ...
         * @return
         */
        public static SingleCalculationSetting create(Random rnd, final HashMap<TermID, Double> wantedActiveTerms,
            double alphaStudySet, Ontology ontology, AssociationContainer assoc)
        {
            ...
            HashMap<TermID, StudySet> wantedActiveTerm2StudySet = new HashMap<>();
            ...
            HashSet<ByteString> fp = new HashSet<>();
            for (ByteString gene : allGenes) {
                if (newStudyGenes.contains(gene)) {
                    continue;
                }
                if (rnd.nextDouble() < alphaStudySet) {
                    fp.add(gene);
                }
            }

            /* true -> false (beta, false negative) */
            HashSet<ByteString> fn = new HashSet<>();
            for (TermID t : wantedActiveTerms.keySet()) {
    ...
    public void testBayes2GOSimple()
    {
        ...
        SingleCalculationSetting scs =
            SingleCalculationSetting.create(new Random(1), wantedActiveTerms, 0.25, ontology, assoc);
    ...
    private static void createInternalOntology(long seed)
    {
        ...
        for (int i = 1; i <= 500; i++) {
            ...
            for (int j = 0; j < numTerms; j++) {
                ...
                assoc.addAssociation(new Association(new ByteString(itemName), new TermID(TermID.DEFAULT_PREFIX, tid)));
            }
        }
    }
    ...
    public static void main(String[] args) throws InterruptedException, IOException
    {
        ...
        // wantedActiveTerms.put(new TermID("GO:0001505"), B2GTestParameter.BETA); /* regulation of neuro transmitter
        // levels */
        ...
        StudySet newStudyGenes = new StudySet("study");
        for (TermID t : wantedActiveTerms.keySet()) {
            System.out.println(t.toString() + " genes=" + wantedActiveTerm2StudySet.get(t).getGeneCount() + " beta="
                + wantedActiveTerms.get(t));
        ...
        if (true) {
            for (ByteString gene : newStudyGenes) {
                if (rnd.nextDouble() < betaStudySet) {
                    fn.add(gene);
                }
            }
        } else {
            /*
             * If this path is enabled, we support more than one term.
             */
            for (TermID t : wantedActiveTerms.keySet()) {
                double beta = wantedActiveTerms.get(t);
                StudySet termStudySet = wantedActiveTerm2StudySet.get(t);
        ...
        /**** Write out the graph ****/
        // {
        HashSet<TermID> allTermIDs = new HashSet<>();
        for (Term t : graph) {
        ...
        GODOTWriter.writeDOT(graph, new File("toy-all.dot"), null, allTermIDs, new AbstractDotAttributesProvider()
        {
            ...
            public String getDotNodeAttributes(TermID id)
            {
                ...
                str.append("\\n");
                str.append(studySetEnumerator.getAnnotatedGenes(id).totalAnnotatedCount() + "/"
                    + allEnumerator.getAnnotatedGenes(id).totalAnnotatedCount());
                str.append("\"");
        ...
        GODOTWriter.writeDOT(graph, new File("toy-induced.dot"), null, wantedActiveTerms.keySet(),
        ...
        //
        // GODOTWriter.writeDOT(graph, new File("toy-result-avg.dot"), null, wantedActiveTerms.keySet(), new
        // IDotNodeAttributesProvider()
        // {
        // public String getDotNodeAttributes(TermID id)
        // {
        // StringBuilder str = new StringBuilder(200);
        ...
        ICalculation calc)
    {
        ...
        if (calc instanceof Bayes2GOCalculation) {
            if (result instanceof Bayes2GOEnrichedGOTermsResult) {
                ...
                // if (!(((Bayes2GOCalculation)calc).noPrior)) wantedScore += wantedActiveTerms.size() *
                // Math.log(p/(1.0-p));
                System.out.println("Score of the given set is " + wantedScore);

                HashSet<TermID> terms = new HashSet<>(wantedActiveTerms.keySet());
        ...
        ArrayList<AbstractGOTermProperties> resultList = new ArrayList<>();
        for (AbstractGOTermProperties prop : result) {
            resultList.add(prop);
        }
        Collections.sort(resultList);
        ...
        // if (prop!=null)
        // System.out.println(" " + prop.goTerm.getIDAsString() + "/" + prop.goTerm.getName() + " " + (/*1.0f -
        // */prop.p_adjusted) + ")");
        // else
...

        {
            ...
            HashSet<TermID> terms = new HashSet<>();
            ...
            for (AbstractGOTermProperties prop : resultList) {
                if (wantedActiveTerms.containsKey(prop.goTerm.getID())) {
                    System.out.println(" " + prop.goTerm.getIDAsString() + "/" + prop.goTerm.getName() + "   "
                        + (/* 1.0f - */prop.p_adjusted) + " rank=" + rank + " beta="
                        + wantedActiveTerms.get(prop.goTerm.getID()));
                }
                rank++;
            }
            ...
            for (AbstractGOTermProperties prop : resultList) {
                if (prop.p_adjusted < 0.9) {
                    ...
                    // System.out.println(" " + prop.goTerm.getIDAsString() + "/" + prop.goTerm.getName() + " " +
                    // (/*1.0f - */prop.p_adjusted) + " rank=" + rank);
                }
                rank++;
            }
    ...
    // static private void scoreDistribution(Bayes2GOCalculation calc, GOTermEnumerator allEnumerator, PopulationSet
    // popSet, StudySet studySet)
    // {
    ...
    private static void loadOntology() throws InterruptedException, IOException
    {
        ...
        ws.setAssociationPath(
            "http://cvsweb.geneontology.org/cgi-bin/cvsweb.cgi/go/gene-associations/gene_association.fb.gz?rev=HEAD");
