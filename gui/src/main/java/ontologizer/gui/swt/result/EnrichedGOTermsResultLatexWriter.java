...
{
    /**
     * Converts the given double value to a proper latex formated version.
     *
     * @param val
     * @return
     */
        ...
        try {
            ...
            for (AbstractGOTermProperties props : sortedProps) {
                ...
                out.println(props.annotatedStudyGenes + "("
                    + String.format("%.1f", (double) props.annotatedStudyGenes / result.getStudyGeneCount() * 100)
                    + "\\%)");
                out.println(" & ");
                out.println(props.annotatedPopulationGenes + "(" + String.format("%.1f",
                    (double) props.annotatedPopulationGenes / result.getPopulationGeneCount() * 100) + "\\%)");
                out.println(" \\\\");
            ...
            out.println(String.format("\\caption{" +
                ...
                result.getCalculationName(),
                result.getCorrectionName() != null ? ("/" + result.getCorrectionName()) : "",
                result.getPopulationGeneCount(), result.getStudyGeneCount()));
            out.println("\\end{table}");
            out.println("\\begin{thebibliography}{1}");
            out.println(
                "\\bibitem{Ontologizer2008} Sebastian Bauer, Steffen Grossmann, Martin Vingron, Peter N. Robinson. Ontologizer 2.0 -- a multifunctional tool for GO term enrichment analysis and data exploration. \\emph{Bioinformatics}, \\textbf{24}(14): 1650--1651.");
