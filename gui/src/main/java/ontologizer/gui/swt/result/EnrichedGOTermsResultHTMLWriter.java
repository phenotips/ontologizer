    ...
    public static void write(EnrichedGOTermsResult result, File htmlFile, File dotFile)
    {
        try {
            ...
            for (Entry<TermID, Node> entry : data.term2node.entrySet()) {
                ...
                double w = (Double) n.getAttributeValue(GrappaConstants.WIDTH_ATTR) * 72;
                double h = (Double) n.getAttributeValue(GrappaConstants.HEIGHT_ATTR) * 72;
                double x1 = cx - w / 2;
                double y1 = cy - h / 2;
                ...
                String coords =
                    String.format("%d,%d,%d,%d", (int) points[0], (int) points[1], (int) points[2], (int) points[3]);
            ...
            if (showMarginals) {
                out.println(
                    "<tr><th>ID</th><th>Name</th><th>Marginal</th><th>Study Count</th><th>Population Count</th>");
            } else {
                out.println(
                    "<tr><th>ID</th><th>Name</th><th>p-Value</th><th>p-Value (Adj)</th><th>Study Count</th><th>Population Count</th>");
            }
            out.println("</tr>");
            ...
            for (AbstractGOTermProperties props : sortedProps) {
                ...
                out.println("<td>");
                out.printf("<a name=\"%s\" href=\"http://www.ebi.ac.uk/ego/DisplayGoTerm?id=%s\">",
                    props.goTerm.getID().id, props.goTerm.getIDAsString());
