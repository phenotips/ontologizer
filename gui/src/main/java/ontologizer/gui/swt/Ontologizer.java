...
public class Ontologizer
{
    ...
    static {
        /* Default */
        workSetList.add("C. elegans", "http://www.geneontology.org/ontology/gene_ontology_edit.obo",
            "http://cvsweb.geneontology.org/cgi-bin/cvsweb.cgi/go/gene-associations/gene_association.wb.gz?rev=HEAD");
        workSetList.add("Fruit Fly", "http://www.geneontology.org/ontology/gene_ontology_edit.obo",
            "http://cvsweb.geneontology.org/cgi-bin/cvsweb.cgi/go/gene-associations/gene_association.fb.gz?rev=HEAD");
        workSetList.add("Mouse", "http://www.geneontology.org/ontology/gene_ontology_edit.obo",
            "http://cvsweb.geneontology.org/cgi-bin/cvsweb.cgi/go/gene-associations/gene_association.mgi.gz?rev=HEAD");
        workSetList.add("Human", "http://www.geneontology.org/ontology/gene_ontology_edit.obo",
            "http://cvsweb.geneontology.org/cgi-bin/cvsweb.cgi/go/gene-associations/gene_association.goa_human.gz?rev=HEAD");
        workSetList.add("Protein Data Bank", "http://www.geneontology.org/ontology/gene_ontology_edit.obo",
            "http://cvsweb.geneontology.org/cgi-bin/cvsweb.cgi/go/gene-associations/gene_association.goa_pdb.gz?rev=HEAD");
        workSetList.add("Rice", "http://www.geneontology.org/ontology/gene_ontology_edit.obo",
            "http://cvsweb.geneontology.org/cgi-bin/cvsweb.cgi/go/gene-associations/gene_association.gramene_oryza.gz?rev=HEAD");
        workSetList.add("Yeast", "http://www.geneontology.org/ontology/gene_ontology_edit.obo",
            "http://cvsweb.geneontology.org/cgi-bin/cvsweb.cgi/go/gene-associations/gene_association.sgd.gz?rev=HEAD");
    }

    /**
     * This is the action which is executed on a download click within the workset window.
     */
    private static ISimpleAction downloadAction =
        new ISimpleAction()
        {
            ...
            {
                try {
                    if (workSet.getSelectedAddress() == null) {
                        return;
                    }
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        if (os.contains("Linux")) {
            /*
             * We are running on linux which requires the MOZILLA_FIVE_HOME variable set. If this is not done, we do it
             * now.
             */
            ...
            if (mozHome == null) {
                ...
                if (xulRunnerDir.exists()) {
                    env.SetEnv.setenv("MOZILLA_FIVE_HOME", "/usr/lib/xulrunner");
                } else {
                    env.SetEnv.setenv("MOZILLA_FIVE_HOME", "/usr/lib/mozilla");
                }
            }
        }
        ...
        try {
            if (!helpFolderFile.exists()) {
                /*
                 * The help folder doesn't exists, so try to copy the files to a directory
                 */
        ...
        main.addSimilarityAction(new ISimpleAction()
        {
            ...
            public void act()
            {
                ...
                if (list.size() > 1) {
                    ...
                    final SimilarityThread newThread =
                        new SimilarityThread(display, calledWhenFinished, result, studySetList, workSet);
                    result.addCloseAction(new ISimpleAction()
                    {
                        @Override
                        public void act()
        ...
        /*
         * When a new method is selected check if the new method supports MTC and update the state of the MTC selector
         * accordingly.
         */
        ...
        FileCache.addUpdateCallback(new FileCacheUpdateCallback()
        {
            ...
            /**
             * Indicates that a refresh is pending (no need to issue this twice)
             */
        ...
        ICalculation calc =
            CalculationRegistry.getCalculationByName(p.get("method", CalculationRegistry.getDefault().getName()));
        if (calc == null) {
            calc = CalculationRegistry.getDefault();
        }
        ...
        /*
         * Dispose the result windows but they have to be copied into a separate array before, as disposing the window
         * implicates removing them from the list
         */
        ...
        /*
         * Ensure that all threads are finished before the main thread disposes the device.
         */
