...
public class GODOTWriter
{
     ...
     * @param provider should provide for every property an appropiate id.
     */
    public static void writeDOT(Ontology graph, File file, TermID rootTerm, Set<TermID> terms,
     ...
     * @param ignoreTerms
     */
    public static void writeDOT(final Ontology graph, File file, TermID rootTerm, Set<TermID> terms,
        final IDotAttributesProvider provider, final String graphAttrs, final boolean reverseDirection,
        final boolean edgeLabels, Set<TermID> ignoreTerms)
    {
        ...
        try {
            graph.getGraph().writeDOT(new FileOutputStream(file), nodeSet, new DotAttributesProvider<Term>()
            {
                ...
                {
                    return "/* Generated with Ontologizer " + BuildInfo.getVersion() + " build "
