...
public class StudySetSampler
{
    ...
    /**
     * Adds a sample of a desired size from a list of genes to an existing sutdy set
     *
     * @param desiredSize the number of genes to sample from the available genes
     * @param studySet the study set to which to add the sampled genes
     * @param repository the repository of available genes from which to draw the sample
     */
    ...
    /**
     * @param baseStudySet The study set from which to sample
     */
     ...
     *
     * @param desiredSize the desired size of the sampled subset
    ...
    /**
     * Samples a random sub study set of random size from the class's base study set. The size is a uniformly
     * distributed fraction of the base study set's size.
     *
    ...
    /**
     * Samples a random sub study set from the class's base study set. In the sampled set the presence of some terms can
     * be controlled as follows: The terms to control are listed in the call and for each term the percentage at which
     * its genes should be present in the sampled study set has to be specified. Additionally, the percentage of genes
     * that should be sampled from the rest has to be given. Observe that there is a fundamental difference between the
     * sampling of the listed genes and the sampling from the rest. For each listed gene, the method ensures that the
     * specified percentage of its genes will be present in the sampled set. For the rest of the genes the specified
     * fraction is sampled from the whole set, giving no control over the fraction sampled from an individual term.
     *
     * @param graph the underlying ontology structure
     * @param associationContainer the gene-term associations
     * @param enrichRule the rule specifying how to enrich terms
     * @param failIfZero if set to true, no study set will be created if a desired study set size would be zero. In this
    ...
    /**
     * An alternative way to over-represent terms by increasing the sampling probability for the genes annotated to the
     * terms.
     *
     * @param desiredSize the desired size of the sample
     * @param termsToOverRepresent a list of terms to over-represent
     * @param OverRepresentationRatio the ratio specifying how much higher the sampling probability should be for the
     *            genes of the specified terms
        ...
        TermID[] termsToOverRepresent, double OverRepresentationRatio)
    {
        ...
        /*
         * We will create two HashSets: One containing the genes to over-represent, the other containing the rest.
         */
        ...
        WeightedUrn<ByteString> termUrn =
            new WeightedUrn<ByteString>(genesToOverRepresent, genesRest, OverRepresentationRatio);
