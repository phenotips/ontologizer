...
/**
 * This class holds all gene names of a study and their associated (optional) descriptions. The names are extracted
 * directly by the constructor given study file. The class implements the Iterable interface so you can conveniently
 * iterate over all includes gene names.
 *
...
public class StudySet implements Iterable<ByteString>
{
    ...
    /**
     * Constructs a study set with genes listed in the specified file.
     *
    ...
    /**
     * Returns the associated attribute (i.e., description) of a gene within a study set.
     *
     * @param name
     * @return
     */
    ...
    /**
     * After all the calculation has been done, you should call this method in order to allow the garbage collector to
     * free the enumerator and counter associated memory.
     */
     ...
     * @param geneName
     * @param description
     * @note After a gene has been added, countGOTerms() should be called again.
     */
     ...
     *
     * @param geneName the name of the gene which inclusion should be checked.
    ...
    /**
     * Returns a hashset containing all the gene names represented by this study set. Note that the returned hashset is
     * decoupled from the study set, so any change within the hashset doesn't have any effect on the study set's genes.
     *
     * @return
     */
    ...
    /**
     * Some datasets contain gene entries which refer to the same gene but differ in their name (i.e. synonyms). This
     * method filters them out by using the association container.
     *
        ...
        for (ByteString geneName : this.gene2Attribute.keySet())
        {
            ...
            } else
            {
                if (associationContainer.isObjectSymbol(geneName)) {
                    ...
                    numObjectID++;
                } else if (associationContainer.isSynonym(geneName)) {
                    numSynonyms++;
                }
            }
        }
    ...
    /**
     * Count the number of genes annotated for every term. Multiple calls to this method are fast, if the gene set has
     * not been changed inbetween.
     *
     * @param goTerms
     * @param associationContainer
     */
    ...
    public GOTermEnumerator enumerateGOTerms(Ontology graph, AssociationContainer associationContainer,
        Set<ByteString> evidences)
    {
        return enumerateGOTerms(graph, associationContainer, evidences, null);
    }

    /**
     * Enumerate genes annotated for every term. Multiple calls to this method are fast, if the gene set has not been
     * changed in-between.
     *
     * @param graph
     * @param associationContainer
     * @param evidences which type of annotation to respect
     * @param remover an instance to decide whether a given term is removed from the annotation set.
     * @return
     */
    public synchronized GOTermEnumerator enumerateGOTerms(Ontology graph, AssociationContainer associationContainer,
    ...
    /**
     * Generate a studyset which contains desiredSize random selected genes of the population.
     *
     * @param desiredSize specifies the desired size of the studyset.
        ...
        try
        {
            ...
            for (ByteString gene : this)
            {
                ...
                if (geneAssociations != null)
                {
                    ...
                    /*
                     * indirect annotation are those located upstream (nearer to the root), i.e., those inferred by the
                     * propagation annotation rule
                     */
    ...
    /**
     * Applies the given filter to this study set. Only simple mapping is supported for now. Genes which have no entry
     * withing the map will remain unaffected while genes which map to "-" will be discarded.
     *
