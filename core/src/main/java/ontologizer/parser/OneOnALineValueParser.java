...
public class OneOnALineValueParser extends OneOnALineParser
{
    ...
    protected void processLine(String line, IParserCallback callback)
    {
        ...
        if (sfields.length < 2) {
            throw new IllegalArgumentException("Number of colums is smaller than two. Affected line contains \"" + line
                + "\"");
        }
        ...
        if (sfields.length > 2) {
            itemAttribute.description = new StringBuilder(sfields[2]).toString();
        } else {
