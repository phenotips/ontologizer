...
/**
 * Class with static methods to calculate probabilities according to the hypergeometric distribution.
 *
...
public class Hypergeometric
{
    ...
    /**
     * This Vector contains log factorials for each index value and acts as a cache.
     */
     ...
     * </P>
     * <UL>
     * <LI>We set up the problem as sampling a set of genes (study genes, for instance, the set of upregulated genes in
     * some microarray experiment) from a larger set of genes (say, the set of all genes of a species). The sampling is
     * done without replacement.</LI>
     * <LI>For each GO term, we can conceive of the population as being divided into genes annotated to this term and
     * those genes that are not annotated to the term.</LI>
     * <LI>The probability of having a certain number of terms annotated to the term in the study set can then be
    ...
    {
        /*
         * Study group cannot be larger than population. If this happens there is probably something wrong with the
         * input data, but returning 1.0 prevents confusing and wrong output.
         */
        if (k >= n) {
            return 1.0;
        }
    ...
    /**
     * Calculates the probabilty that if you draw n balls from an urn without replacement containing N balls where M
     * among them are white (and so N-M are black) you will get x white balls.
     *
    ...
    public double dhyper(int x, int N, int M, int n)
    {
        ...
        /*
         * Of course it is also not possible to draw more white balls than the number of drawings. The probability is 0.
         */
        if (x > n) {
            return 0;
        }
        ...
        /*
         * Last but not least, it is also not possible to draw more black balls than there are within the urn.
         */
        if (n - x > N - M) {
            return 0;
        }
    ...
    /**
     * Calculates P(X > x) where X is the hypergeometric distribution with indices N,M,n. If lowerTail is set to true,
     * then P(X <= x) is calculated.
     *
     * @param x number of white balls drawn without replacement
     * @param N number of balls in the urn
     * @param M number of white balls in the urn
     * @param n number of balls drawn from the urn
     * @param lowerTail defines if the lower tail should be calculated, i.e., if the parameter is set to true then
     *            P(X<=x) is calculated, otherwise P(X > x) is calculated.
     * @return the probability
     */
    ...
    /**
     * return the log factorial of i. Use a cache to avoid repeatedly calculating this. If we have a cache miss, fill up
     * all values from the last valid cache value to the value we currently need.
     */
    ...
    /**
     * Initialize the object lfactorial, which will act as a cache for log factorial calculations.
     */
