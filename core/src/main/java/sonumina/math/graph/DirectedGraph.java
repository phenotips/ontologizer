...
public class DirectedGraph<VertexType> extends AbstractGraph<VertexType> implements Iterable<VertexType>
{
    ...
    /**
     * Adds the given vertex to the graph. Nothing happens if the graph already contains the vertex.
     *
     * @param vertex
     */
    public void addVertex(VertexType vertex)
    {
        if (!this.vertices.containsKey(vertex)) {
            VertexAttributes<VertexType> va = new VertexAttributes<>();
            ...
            for (Edge<VertexType> i : va.inEdges) {
                for (Edge<VertexType> o : va.outEdges) {
                    if (!hasEdge(i.getSource(), o.getDest())) {
                        addEdge(new Edge<>(i.getSource(), o.getDest()));
                    }
                }
            }
    ...
    {
        DirectedGraph<VertexType> copy = new DirectedGraph<>();
        ...
        while (nodeIt.hasNext()) {
            ...
            while (descIt.hasNext()) {
                copy.addEdge(new Edge<>(node, descIt.next()));
            }
        }
        return copy;
    }
     ...
     * @throws IllegalArgumentException if the edge is a link between two vertices which haven't been added to the
     *             graph.
     */
    public void addEdge(Edge<VertexType> edge)
    {
        ...
        /*
         * Ensure that the arguments are valid, i.e. both source and destination must be vertices within the graph
         */
        if (vaSource == null || vaDest == null) {
            throw new IllegalArgumentException("Error when trying to add edge between source: " + vaSource
                + " and destination: " + vaDest + ".");
        }
    ...
    public void removeConnections(VertexType source, VertexType dest)
    {
        ...
        HashSet<Edge<VertexType>> deleteMe = new HashSet<>();
        ...
        if (deleteMe.size() > 1) {
            throw new RuntimeException(
                " found more than one edge to delete! (" + deleteMe.size() + ") --> " + deleteMe);
        }
        for (Edge<VertexType> edge : deleteMe) {
            // System.out.print("rem: "+edge.getSource()+" -> "+edge.getDest());
    ...
    /**
     * Returns the iterator which can be used for conveniently iterating over all vertices contained within the graph.
     *
     * @return the iterator.
     */
    ...
    /**
     * Returns the edge connecting source to dest. As multi graphs are not supported this is unique.
     *
     * @param source
     * @param dest
    ...
    /**
     * Returns the iterator to iterate through all edges going into the given object (i.e. all edges where the object is
    ...
    {
        VertexAttributes<VertexType> va = this.vertices.get(t);
        assert (va != null);
    ...
    {
        VertexAttributes<VertexType> va = this.vertices.get(vt);
        assert (va != null);

        List<VertexType> ancestors = new LinkedList<>();
    ...
    /**
     * Returns clustering coefficient as described in the paper by Watts and Strogatz (1998 Nature).
     *
     * @param v
    ...
    public double getClusteringCoefficient(VertexType v)
    {
         ...
         */
        HashSet<VertexType> neighborhood = new HashSet<>();
        ...
        /*
         * For isolated nodes we set the CC to 0. This is kind of standard, even though in Brandes & Erlebach (Network
         * Analysis) 2005 these are set to one. A discussion of this can be found in: Marcus Kaiser; New Journal of
         * Physics (2008); "Mean clustering coefficients: the role of isolated nodes and leafs on clustering measures
        ...
        /*
         * Now determine the number of links inside the neighborhood.
         */
        ...
        double C = numEdgesNeighborhood / denominator;
        return C;
    }

    /**
     * Returns the iterator to iterate through all edges going out of given object. (i.e. all edges where the object is
     * the edge's source)
     *
     * @param t
     * @return
    ...
    public Iterator<VertexType> getChildNodes(VertexType vt)
    {
        ...
        List<VertexType> descendant = new LinkedList<>();
        for (Edge<VertexType> e : va.outEdges) {
    ...
    public Iterable<VertexType> getDescendantVertices(VertexType vt)
    {
        ...
        List<VertexType> descendant = new ArrayList<>(va.outEdges.size());
        for (Edge<VertexType> e : va.outEdges) {
            descendant.add(e.getDest());
        }
        return descendant;
    ...
    /**
     * Calculates the shortest path from the given vertex to all vertices. Note that negative weights are not supported!
     *
     * @param vertex defines the source
     * @param againstFlow if specified the path is walked against the direction of the graph
     * @param visitor object implementing IDistanceVisitor which can be used to process the results
     */
    public void singleSourceShortestPath(VertexType vertex, boolean againstFlow, IDistanceVisitor<VertexType> visitor)
    {
        /**
         * This class implements some meta information needed by Dijkstra's algorithm.
         *
         * @author Sebastian Bauer
         */
        ...
        /*
         * Within the priority queue we maintain the vertices which has been already discovered by the algorithm. TODO:
         * Get rid of Java's PriorityQueue by using a better suited data structure
         */
        PriorityQueue<VertexExtension> queue = new PriorityQueue<>();
        HashMap<VertexType, VertexExtension> map = new HashMap<>();
        ...
        while (!queue.isEmpty()) {
            ...
            if (againstFlow) {
                edgeIter = getInEdges(next.vertex);
            } else {
                edgeIter = getOutEdges(next.vertex);
            }
            ...
            while (edgeIter.hasNext()) {
                ...
                if (neighbourExt == null) {
                    ...
                    /*
                     * Would the edge from the current vertex to the neighbour make the path to the neighbour shorter?
                     */
        ...
        for (Entry<VertexType, VertexExtension> v : map.entrySet()) {
            /*
             * Build the path by successively traversing the path from the current destination through the stored
             * ancestors (parents)
             */
            LinkedList<VertexType> ll = new LinkedList<>();
    ...
    public void bf(VertexType source, int weightMultiplier, IDistanceVisitor<VertexType> visitor)
    {
        ...
        for (int i = 0; i < this.vertices.size(); i++) {
            ...
            /* Edge loop */
            for (Entry<VertexType, VertexAttributes<VertexType>> ent : this.vertices.entrySet()) {
        ...
        for (Entry<VertexType, VertexExtension> v : map.entrySet()) {
            ...
            VertexExtension curVe = v.getValue();
            do {
                ll.addFirst(curVe.vertex); /* FIXME: (VertexType) is for the java compiler */
                curVe = map.get(curVe.parent);
            } while (curVe != null);
    ...
    public int getMaxDegree()
    {
        ...
        for (VertexType vertex : this.vertices.keySet()) {
            ...
            if (degreeIn != degreeOut) {
                throw new RuntimeException("Vertex " + vertex + " has indegree:" + degreeIn + " and outdegree:"
                    + degreeOut);
            }
            if (degreeOut > max) {
                max = degreeOut;
    ...
    {
        return subGraph(new HashSet<>(verticesToBeIncluded));
    }

    /**
     * Returns a subgraph of the graph that includes all given vertices. Edges are included only, if it is spanned
     * between two vertices in the given set.
     *
     * @param verticesToBeIncluded
     * @return
     */
    ...
    {
        DirectedGraph<VertexType> graph = new DirectedGraph<>();
    ...
    /**
     * Returns the path-transitivity-maintaining transitive closure of a subgraph that contains the given vertices.
     *
     * @param verticesToBeIncluded
     * @return
     */
    public DirectedGraph<VertexType> transitiveClosureOfSubGraph(final Set<VertexType> verticesToBeIncluded)
    {
        ...
        final DirectedGraph<VertexType> graph = new DirectedGraph<>();
            ...
            bfs(v1, false, new IVisitor<VertexType>()
            {
                ...
                public boolean visited(VertexType vertex)
                {
                    if (verticesToBeIncluded.contains(vertex)) {
                        graph.addEdge(new Edge<>(v1, vertex));
                    }
                    return true;
                };
            });
    ...
    /**
     * Returns a sub graph with selected vertices, in which path relationships are maintained. Basic version.
     *
     * @return
     */
    private DirectedGraph<VertexType> compactedSubgraph(final Set<VertexType> verticesToBeIncluded)
    {
        ...
        /*
         * Note that we iterate here over the nodes the this instance and not over the duplicated graph. We will remove
         * nodes from there therefore iterating over those nodes is node safe.
         */
    ...
    /**
     * Returns a sub graph with selected vertices, in which path relationships are maintained.
     *
     * @param verticesToBeIncluded
     * @return
     */
    public DirectedGraph<VertexType> pathMaintainingSubGraph(Set<VertexType> verticesToBeIncluded)
    {
        ...
        do {
            ...
            transitivitySubGraph = new DirectedGraph<>();
            for (VertexType v : verticesToBeIncluded) {
                transitivitySubGraph.addVertex(v);
            }

            /*
             * Now add edges to the reduced graph structure, i.e., leave out edges that are redundant
             */
            for (VertexType v : verticesToBeIncluded) {
                Set<VertexType> vUpperVertices = transitiveClosure.getVerticesOfUpperInducedGraph(null, v);
                LinkedList<VertexType> parents = new LinkedList<>();
                ...
                /*
                 * Construct the upper graph by using only the parents. Always leave out a single parent. If that edge
                 * is redundant, the number of nodes in this newly created graph differs only by one.
                 */
                for (VertexType p : parents) {
                    HashSet<VertexType> pUpperVertices = new HashSet<>();
    ...
    /**
     * Merge equivalent vertices. First vertex given will be the representative and thus we inherit all the edges of the
     * other equivalent vertices.
     *
     * @param vertex1
     * @param eqVertices
     */
