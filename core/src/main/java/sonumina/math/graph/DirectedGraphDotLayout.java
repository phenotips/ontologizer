...
/**
 * An extension of DirectedGraphLayout which uses dot. This is a weird construct.
 *
...
public class DirectedGraphDotLayout<T> extends DirectedGraphLayout<T>
{
    ...
    private DirectedGraphDotLayout(DirectedGraph<T> graph, IGetDimension<T> dimensionCallback,
    ...
    /**
     * Walks through the graph hierarchy and emits the positions for each node using the supplied positionCallback.
     *
     * @param e
     * @param miny
     */
    private void emitPosition(Element e, int miny)
    {
        switch (e.getType()) {
            case GrappaConstants.NODE: {
                ...
                double w = (Double) node.getAttributeValue(GrappaConstants.WIDTH_ATTR) * DPI;
                double h = (Double) node.getAttributeValue(GrappaConstants.HEIGHT_ATTR) * DPI;
    ...
    /**
     * Lays out the graph using DirectedGraphDotLayout. If this for some reasons fail, the limited algorithm of
     * DirectedGraphLayout is employed.
     *
     * @param graph
