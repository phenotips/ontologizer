...
{
    /**
     * This interface is used as a callback mechanism by different search methods.
     *
     * @author Sebastian Bauer
     */
    public static interface IVisitor<VertexType>
    {
         ...
         *
         * @param goTermID
         * @return false if algorithm should be stopped (i.e. no further calls to this method will be issued) otherwise
         *         true
         */
    ...
    /**
     * Performs a breadth-first search onto the graph starting at a given vertex. Vertices occurring in loops are
     * visited only once.
     *
     * @param vertex defines the vertex to start with.
     * @param againstFlow the bfs in done against the direction of the edges.
     * @param visitor a object of a class implementing IVisitor. For every vertex visited by the algorithm the
     *            visitor.visited() method is called. Note that the method is also called for the vertex represented by
     *            <code>vertex</code> (the one given as parameter to this method).
     * @see IVisitor
     */
     ...
     *
     * @param vertex defines the vertex to start with.
     * @param grabber a object of a class implementing INeighbourGrabber which returns the nodes which should be visited
     *            next.
     * @param visitor a object of a class implementing IVisitor. For every vertex visited by the algorithm the
     *            visitor.visited() method is called. Note that the method is also called for the vertex represented by
     *            vertex.
    ...
    /**
     * Performs a breadth-first search onto the graph starting at a given set of vertices. Vertices occurring in loops
     * are visited only once.
     *
     * @param initial defines the set of vertices to start with.
     * @param againstFlow the bfs in done against the direction of the edges.
     * @param visitor a object of a class implementing IVisitor. For every vertex visited by the algorithm the
     *            visitor.visited() method is called. Note that the method is also called for the vertices specified by
     *            initialSet (in arbitrary order)
     * @see IVisitor
     */
    ...
    public void bfs(Collection<VertexType> initial, INeighbourGrabber<VertexType> grabber, IVisitor<VertexType> visitor)
    {
        ...
        while (!queue.isEmpty())
        {
            ...
            /*
             * Add not yet visited neighbors of old head to the queue and mark them as visited.
             */
    ...
    private void getDFSShotcutLinks(VertexType v, HashMap<VertexType, VertexType> map, HashSet<VertexType> visited,
        ArrayList<VertexType> upwardQueue, INeighbourGrabber<VertexType> grabber, IVisitor<VertexType> visitor)
    {
        visitor.visited(v);
    ...
    /**
     * Returns the vertices in a topological order. If the length of the returned differs from the number of vertices we
     * have a cycle.
     *
     * @return
     */
    ...
    /**
     * The provider class for node and edge attributes that are used in the dot file.
     *
     * @author Sebastian Bauer
     * @param <VertexType>
     */
     ...
     */
    public void writeDOT(OutputStream fos, Iterable<VertexType> nodeSet,
