 ...
 * OntologizerCMD.java
 * <P>
 * This class is used by the command-line version but not by the GUI version of the Ontologizer. Its purpose is to get
 * the command line arguments and create a new instance of the class Controller, which in turn takes care of the XML
 * parsing and HTML or XML output.
 * </P>
 * <P>
 * Note that the static main method is the only thing that is used in this class.
 * </P>
 *
...
{
    /**
     * The required argument stuff of the jakarta cli didn't work as expected, so we have to do this manually. If the
     * specified argument is not found an appropriate error message is written the program exited.
     *
     * @param cmd
     * @param name
     * @return
    ...
    private static String getRequiredOptionValue(CommandLine cmd, char name)
    {
        ...
        if (val == null) {
            System.err.println(
                "Aborting because the required argument \"-" + name + "\" wasn't specified! Use the -h for more help.");
            System.exit(-1);
        }
        return val;
    }
    ...
    /**
     * Extract command line arguments and use them to direct program flow using a Controller instance.
     *
     * @param args an array of Strings that will passed to a myCommandLine object
     * @see myCommandLine
     * @see OntologizerCore
     */
    public static void main(String[] args)
    {
        ...
        try {
            ...
            for (String calculation : calculations) {
                ...
                calHelpStrBuilder.append(calculation);
                calHelpStrBuilder.append("\"");

                /* Add default identifier if it is the default correction */
                if (CalculationRegistry.getDefault() == CalculationRegistry.getCalculationByName(calculation)) {
            ...
            for (String mtc : mtcs) {
                if (TestCorrectionRegistry.getCorrectionByName(mtc) instanceof IResampling) {
                    resamplingBasedMTCsExists = true;
                }

                mtcHelpStrBuilder.append("\"");
                mtcHelpStrBuilder.append(mtc);
                mtcHelpStrBuilder.append("\"");

                /* Add default identifier if it is the default correction */
                if (TestCorrectionRegistry.getDefault() == TestCorrectionRegistry.getCorrectionByName(mtc)) {
            ...
            options.addOption(
                new Option("g", "go", true, "File containig GO terminology and structure (.obo format). Required"));
            options.addOption(
                new Option("a", "association", true, "File containing associations from genes to GO terms. Required"));
            options.addOption(
                new Option("p", "population", true, "File containing genes within the population. Required"));
            options.addOption(new Option("s", "studyset", true,
                "File of the study set or a directory containing study set files. Required"));
            options.addOption(
                new Option("i", "ignore", false, "Ignore genes to which no association exist within the calculation."));
            options.addOption(new Option("c", "calculation", true, calHelpString));
            options.addOption(new Option("m", "mtc", true, mtcHelpString));
            options.addOption(new Option("d", "dot", true,
            ...
            options.addOption(new Option("n", "annotation", false,
                "Create an additional file per study set which contains the annotations."));
            options.addOption(new Option("f", "filter", true,
                "Filter the gene names by appling rules in a given file (currently only mapping supported)."));
            options.addOption(
                new Option("o", "outdir", true, "Specfies the directory in which the results will be placed."));

            if (resamplingBasedMTCsExists) {
                options.addOption(new Option("r", "resamplingsteps", true,
                    "Specifies the number of steps used in resampling based MTCs"));
                options.addOption(new Option("t", "sizetolerance", true,
                    "Specifies the percentage at which the actual study set size and " +
            ...
            if (cmd.hasOption("v")) {
                System.out
                    .println("Ontologizer " + BuildInfo.getVersion() + " (Build: " + BuildInfo.getBuildString() + ")");
                System.out.println();
            ...
            if (arguments.calculationName != null) {
                boolean found = false;
                for (String calculation : calculations) {
                    if (arguments.calculationName.equals(calculation)) {
                        found = true;
                        break;
                    }
                }
                ...
                if (!found) {
                    String addInfo = " Did you mean perhaps \""
                        + SmithWaterman.findMostSimilar(calculations, arguments.calculationName) + "\"?";
                    System.err
                        .println("Given calculation method " + arguments.calculationName + " wasn't found!" + addInfo);
                    System.exit(-1);
                }
            }
            ...
            if (arguments.correctionName != null) {
                boolean found = false;
                for (String mtc : mtcs) {
                    if (arguments.correctionName.equals(mtc)) {
                        found = true;
                        break;
                    }
                }
                ...
                if (!found) {
                    String addInfo = " Did you mean perhaps \""
                        + SmithWaterman.findMostSimilar(mtcs, arguments.correctionName) + "\"?";
            ...
            if (dotStr != null && dotStr.length() > 0) {
                ...
                if (dotOpts.length > 0) {
                    try {
                        ...
                        System.err.println(
                            "The argument specified to the 'dot' option isn't given in a known floatingpoint format!");
                        System.exit(-1);
                    }

                    if (!(alpha > 0 && alpha <= 0.99)) {
                        System.err.println(
                            "The argument given to the 'dot' option is not in a valid range (between 0 and 0.99)!");
                        System.exit(-1);
                    }
                    ...
                    if (dotOpts.length > 1) {
                        try {
                            ...
                            try {
                                ...
                                System.err.println(
                                    "The 2nd argument given to the 'dot' option is no valid GO Term ID format (eighter the \"GO:\" string followed by seven digits or a plain integer)!");
                                System.exit(-1);
                            }
                        }
                    }
            ...
            while ((studySetResult = controller.calculateNextStudy()) != null) {
                ...
                if (createAnnotations) {
                    ...
                    studySetResult.getStudySet().writeSetWithAnnotations(controller.getGoGraph(),
                        controller.getGoAssociations(), outFile);
                }
            }
