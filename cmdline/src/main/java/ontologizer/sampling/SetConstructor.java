...
public class SetConstructor
{
    ...
    /**
     * The required argument stuff of the jakarta cli didn't work as expected, so we have to do this manually. If
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        try {
            ...
            options.addOption("a", "association", true,
            ...
            options.addOption("c", "complete", false,
                "The sampled set consists of all annotated genes if set. Ignores -p and -n.");
            options.addOption("n", "nsamples", true,
                "Number of samples to generate. Samples are numbered if necessary. Defaults to 1");
            options.addOption("sm", "subTermMatrix", false,
                "If specified, the output is a matrix in which element (i,j) is set to 1, only if term i is a subterm of term j.");
            ...
            AssociationContainer assocs = new AssociationContainer(assocParser.getAssociations(),
                assocParser.getSynonym2gene(), assocParser.getDbObject2gene());
            ...
            if (enrichMany) {
                System.out
                    .println("You want to construct a large number of study set samples with artificial enrichment");
                ...
                if (k == 2) {
                    ...
                    String[] subPartsFirst = termPercUnsplit.split("\\."); // who would have thought that one needs
                                                                           // double escaping for the dot...
                ...
                ArrayList<ICalculation> calcs = new ArrayList<>();
                ...
                HashSet<TermID> goodTerms = new HashSet<>();
                boolean firstcalc = true;

                for (ICalculation curCalc : calcs) {
                    EnrichedGOTermsResult basicResult =
                        curCalc.calculateStudySet(graph, assocs, completePop, completePop, noCorrection);
                ...
                HashSet<ArrayList<TermID>> sampledTermCombinations = new HashSet<>();
                if (nTupel == 1) // we don't have to take care about the
                                 // namespace
                {
                    KSubsetSampler<TermID> subsetSampler = new KSubsetSampler<>(goodTerms);
                ...
                // same namespace
                {
                    TermID bioproc = new TermID(TermID.DEFAULT_PREFIX, 8150);
                    TermID cellcomp = new TermID(TermID.DEFAULT_PREFIX, 5575);
                    TermID molfunc = new TermID(TermID.DEFAULT_PREFIX, 3674);
                    ...
                    for (TermID tid : mainTerms) {
                        ...
                        HashSet<TermID> thisGoodTerms = new HashSet<>();
                        ...
                        KSubsetSampler<TermID> subsetSampler = new KSubsetSampler<>(thisGoodTerms);
                        sampledTermCombinations.addAll(subsetSampler.sampleManyOrderedWithoutReplacement(
                            nTupel, nCombisPart));
                    }
                }
                ...
                for (ArrayList<TermID> termCombi : sampledTermCombinations) {
                    ...
                    if (sampleStudySet == null) {
                        System.out.println(
                            "At least one term wouldn't be overrepresented due to lack of genes annotated to it. Skipping this study set.");
                    } else {
    ...
    /**
     * Parses the string provided in the -es option to construct the appropriate term enrichment rule.
     *
     * @param esString the string provided in the -es option
     * @param goTerms needed to construct the correct term object
     * @return the term enrichment rule
     * @throws ParseException if string has wrong format
     */
    ...
    /**
     * A clever routine constructing a final name for the sample file from a basename. If the total number of samples
     * requires is larger than one, each name gets extended by a count.
     *
     * @param baseSampleFileName the basename for the sample files
     * @param sampleNumber the number of the actual sample
     * @param totalSamples the total number of samples to construct
        ...
        int sampleNumber, int totalSamples)
    {
        if (totalSamples == 1) {
            ...
            try (Formatter formatter = new Formatter(numberedSampleFileName)) {
    ...
    /**
     * Writes out the files for a specified sample. If listAllTerms is set, it also writes out a directory holding for
     * each term a file with the current annotation.
     *
     * @param sampleFileName the name of the sample file to write
     * @param listAllTerms tells whether term annotation files should be constructed
     * @param graph the underlying graph
     * @param assocs needed to reduce the annotations for the subset
     * @param outDir that's where files get written to
     * @param sample the underlying population set
     ...
     *
     * @param sampleFileName the name of the corresponding sample file
     * @param sampleEnumerator needed to actually construct the lists
     * @param outDir that's where files get written to
     * @throws IOException if writing fails
     */
     ...
     * @param sample holding the gene list to write
     * @param outDir that's where files get written to
     * @throws IOException if writing fails
     */
